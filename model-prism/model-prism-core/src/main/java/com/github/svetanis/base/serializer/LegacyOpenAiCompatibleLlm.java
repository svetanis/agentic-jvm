...
/**
 * Base class for any OpenAI-compatible chat completion API (Groq, Ollama,
 ...
 *
 * <ul>
 * <li>{@link HttpService} - handles HTTP transport
 * <li>{@link OpenAiMessageSerializer} - handles JSON serialization /
 * deserialization
 * </ul>
 *
 * <p>
 ...
 *
 * <p>
 * The {@code protected} injection constructor accepts custom collaborators,
 * which is useful for subclasses and unit tests.
 */
@Deprecated
public class LegacyOpenAiCompatibleLlm extends BaseLlm {
	 ...
	 * @param apiKey    optional API key; if empty, no {@code Authorization} header
	 *                  is sent
	 */
	public LegacyOpenAiCompatibleLlm(String modelName, String apiUrl, Optional<String> apiKey) {
		this(modelName, new DefaultHttpService(apiUrl, apiKey), new DefaultOpenAiMessageSerializer());
	}
	 ...
	 */
	public LegacyOpenAiCompatibleLlm(String modelName, HttpService httpClient, OpenAiMessageSerializer serializer) {
	 ...
	 *
	 * <p>
	 * When {@code stream} is {@code true}, returns a {@link Flowable} that emits
	 * partial text tokens and a final assembled response. When {@code false},
	 * returns a single-element {@link Flowable} with the complete response.
	 */
	@Override
	public Flowable<LlmResponse> generateContent(LlmRequest request, boolean stream) {
		...
		return Flowable.fromCallable(() -> {
			...
			return serializer.deserializeResponse(httpClient.post(requestBody).bodyOrThrow());
		});
	}
	 ...
	 *
	 * <p>
	 * Live (bidirectional) streaming is not supported by OpenAI-compatible APIs.
	 *
