 ...
 * 
 * <p>
 * Unlike {@link OpenAiCompatibleLlm} which uses custom HTTP and JSON
 * serialization adapters, this class utilizes the native
 * {@link ChatCompletionsHttpClient} provided by ADK 1.4.0+, which merges both
 * HTTP transport and JSON mapping into a single highly optimized pipeline.
 * 
 * <p>
 * <strong>WARNING:</strong> This class is provided as an example and is NOT
 * wired by default because the native ADK JSON serialization engine currently
 * outputs uppercase JSON Schema types (e.g., {@code "type": "STRING"}) when
 * processing tools. Strict OpenAI-compatible providers like Groq strictly
 * enforce the JSON Schema standard (which requires lowercase types like
...
public class OpenAiCompatibleLlm extends BaseLlm {
	...
	public OpenAiCompatibleLlm(String modelName, String apiUrl, Optional<String> apiKey) {
		...
		// ChatCompletionsHttpClient automatically appends "/chat/completions"
		// internally
