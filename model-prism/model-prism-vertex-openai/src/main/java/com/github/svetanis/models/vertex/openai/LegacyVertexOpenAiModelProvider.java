 ...
 * at Vertex with a base URL change (e.g. the {@code spring-ai-gateway}
 * pass-through path).</li>
 * <li>You need a partner mode (Llama, Mistral, Anthropic-on-Vertex) without
 * pulling in a model-family-specific SDK.</li>
 * <li>You want a single code path that works against Groq, OpenRouter, Ollama,
 * <em>and</em> Vertex - useful for cross-provider eval suites.</li>
 * </ul>
 * 
 * <h2>Activation</h2>
 * <p>
 * Any model string starting with {@code vertex-openai/}. The prefix is
 * stripped; the remainder is sent to Vertex as the {@code model} field.
 * Vertex's OpenAI surface uses vendor-namespaced model ids:
 * <ul>
 ...
 * {@code GOOGLE_CLOUD_LOCATION} (default {@code us-central1}).
 * </p>
 * 
